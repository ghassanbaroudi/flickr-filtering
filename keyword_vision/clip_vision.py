...
CLIP_TEXT_PROMPTS = (
    "a photograph of a building exterior, monument, or clear architectural structure",
    "a photograph that is not mainly a building: people, nature, documents, floor plans, maps, or unclear architecture",
)
...
class ClipVisionRuntime:
    ...
    def __init__(
        ...
        *,
        ...
        text_tokens = tokenizer(list(CLIP_TEXT_PROMPTS)).to(self.device)
        with torch.no_grad():
    ...
    def classify_pil_batch(self, images: List[object]) -> List[Tuple[str, str, float]]:
        """
        images: list of PIL RGB images (same length as batch).
        Returns (vision_label, vision_reason, p_building) per image.
        p_building = softmax prob for class 0 (building).
        """
        import torch
        ...
        for i in range(len(images)):
            p_yes = float(probs[i, 0].item())
            label = "YES" if int(preds[i].item()) == 0 else "NO"
            reason = f"openclip p(building)={p_yes:.3f}"
            out.append((label, reason, p_yes))
        return out
...
def run_batched_clip(
    ...
    *,
    ...
    Returns ``(results, stopped_early)``.  ``results`` contains one entry per
    processed URL in the same order as *urls*; when ``stopped_early`` is True,
    ``len(results) < len(urls)`` because the run was aborted after
    ``stop_after_errors`` consecutive persistent download failures.

    ``stop_after_errors=0`` disables early stopping (process all URLs).

    If *on_result* is provided it is called immediately after each image is
    scored as ``on_result(global_idx, label, reason, error_or_None)``.  Use
    this for incremental cache writes so that progress is never lost if the
    ...
    # Hard per-URL wall-clock deadline.  This is intentionally shorter than the
    # theoretical max (all retries exhausted) — its purpose is to kill requests
    # that hang indefinitely because the socket timeout never fires (e.g. a server
    # that trickles data or a PIL decode loop on a corrupt image).  One full
    # timeout window plus a 20 s buffer is enough for a normal download while
    # keeping the worst-case stall under two minutes.
    ...
    stopped_early = False
    recent_errors: List[str] = []  # last N error messages for the stop report
    ...
    for start in range(0, n, bs):
        ...
        for i in range(len(chunk_urls)):
            ...
            if not ok_mask[i]:
                ...
                trip: Tuple[str, str, Optional[str]] = ("ERROR", "", err_msg)
                results.append(trip)
                ...
                if on_result is not None:
                    on_result(global_idx, "ERROR", "", err_msg)
            else:
                label, reason, _p = classified[gi]
                gi += 1
                trip = (label, reason, None)
                results.append(trip)
                consecutive_errors = 0
                recent_errors.clear()
                if on_result is not None:
                    on_result(global_idx, label, reason, None)
        ...
        if stop_after_errors > 0 and consecutive_errors >= stop_after_errors:
            err_detail = "; ".join(dict.fromkeys(recent_errors))  # deduplicated, order-preserved
            print(
