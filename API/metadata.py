...
# ----------------- SETUP ------------------- #
# Load env vars from common locations, including existing `flico/env.env`.
api_dir = Path(__file__).resolve().parent
flico_dir = api_dir.parent
project_root = flico_dir.parent
dotenv_candidates = [
    flico_dir / "env.env",
    flico_dir / ".env",
    project_root / ".env",
...
if not API_KEY or not API_SECRET:
    raise ValueError(
        "FLICKR_API_KEY and FLICKR_API_SECRET must be set in one of: "
        "flico/env.env, flico/.env, or project .env"
    )
...
def extract_from_search(photo_dict):
    ...
    return {
        ...
        'accuracy': safe_str(photo_dict.get('accuracy')),
        'context': safe_str(photo_dict.get('context')),
        'comments': 'N/A',
...
def is_outdoor_context(photo_dict):
    """True only for photos with context=2 (outdoors)."""
    return safe_str(photo_dict.get('context')).strip() == '2'


def cleanup_local_metadata_keep_context_2(metadata_directory=metadata_dir):
    """
    Rewrite local metadata CSVs to keep only rows where context == 2.
    Files without a `context` column are skipped.
    """
    print("\n🧹 CLEANING LOCAL METADATA (keep context=2 only)...")
    print("=" * 80)

    csv_files = sorted(metadata_directory.glob("*.csv"))
    if not csv_files:
    ...
    for csv_file in csv_files:
        try:
            ...
            if "context" not in fieldnames:
                print(f"⚠️  SKIP {csv_file.name}: no `context` column")
                skipped_files += 1
                continue

            before = len(rows)
            kept_rows = [r for r in rows if safe_str(r.get("context")).strip() == "2"]
            after = len(kept_rows)
            ...
            print(
                f"✅ {csv_file.name:<55} kept={after:>7,} removed={removed:>7,}"
            )
    ...
    print("=" * 80)
    print(
        f"Done. rows before={total_before:,}, after={total_after:,}, removed={total_removed:,}, "
        f"files skipped(no context)={skipped_files}"
    )
...
def download_metadata(institutions, context_filter=None):
    """Routine 2: Download metadata for prioritized institutions.

    context_filter:
      - None -> keep all photos
      - "2"  -> keep only outdoor photos (context=2)
    """
            ...
            try:
                ...
                for photo_dict in photo_batch:
                    ...
                    if context_filter is not None:
                        photo_context = safe_str(photo_dict.get("context")).strip()
                        if photo_context != str(context_filter):
                            continue
...
if __name__ == "__main__":
    ...
    print("3. Cleanup local CSVs (keep context=2 only)")

    action = input("\nChoose action [1/2/3, default=1]: ").strip() or "1"

    if action == "3":
        cleanup_local_metadata_keep_context_2()
        raise SystemExit(0)
    ...
    if response in ['y', 'yes']:
        context_mode = input(
            "Keep only context=2 photos? (y/n, default=n): "
        ).lower().strip()
        context_filter = "2" if context_mode in ["y", "yes"] else None
        if context_filter == "2":
            print("ℹ️ Download filter: context=2 only")
        else:
            print("ℹ️ Download filter: no context filter (all photos)")

        print("\n🚀 STARTING DOWNLOAD (lowest coverage first)...")
        download_metadata(prioritized_institutions, context_filter=context_filter)
        print("\n🎉 DOWNLOAD COMPLETE!")
    else:
